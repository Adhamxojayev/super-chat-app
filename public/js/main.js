    ...
    for(let user of res.data){
        if(document.cookie.split('=')[1] != user.id){
            friends.innerHTML += `<div class="row sideBar-body" onclick=chat(this)>
              ...
              <span style="color: green;" id="sms" class="time-meta pull-right">online
              </span>
              </div>
              </div>
              </div>
...
async function chat (user) {
    id = user.childNodes[3].childNodes[1].childNodes[1].childNodes[1].id
